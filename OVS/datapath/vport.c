...
void deal_with_int_info(struct vport *vport, struct sk_buff *skb)
{
    ...
    char* data_begin;
    __be32 data_length, skblen, tailroom;
    __be16 iplen, udplen;
    ...
    if (iph->protocol == IPPROTO_UDP)
    {
        ...
        intmd->ingress_port = OVS_CB(pskb)->input_vport->port_no;
        intmd->egress_port = vport->port_no;
        
        do_gettimeofday(&tve);
        end_time = tve.tv_usec;
        intmd->latency = end_time - begin_time;

        //copy payload to actual_data after INT info
        data_begin = skb_transport_header(pskb) + sizeof(struct udphdr);

        data_length = ntohs(iph->tot_len) - skb_network_header_len(pskb) - sizeof(struct udphdr);
        memcpy(actual_data + strlen(actual_data), data_begin, data_length);

        //update IP len & udp len & checksum & frame len
        iph->protocol = IPPROTO_INT;
        iplen = ntohs(iph->tot_len);
        iph->tot_len = htons(ntohs(iph->tot_len) + sizeof(struct int_header) + sizeof(struct int_metadata));
        iph->check = 0;
        iph->check = ip_fast_csum((char*)iph, iph->ihl);

        udplen = ntohs(udp->len);
        udp->len = htons(ntohs(udp->len) + sizeof(struct int_header) + sizeof(struct int_metadata));
        udp->check = 0;

        skblen = pskb->len;
        pskb->len += (sizeof(struct int_header) + sizeof(struct int_metadata));
    }
    else //IPPROTO_INT
    {
        ...
        inth = (struct int_header*)data_begin;
        payload_offset = sizeof(struct int_header) + inth->total_hop_count * sizeof(struct int_metadata);
        inth->total_hop_count += 1;

        data_begin += payload_offset;
        data_length = ntohs(iph->tot_len) - skb_network_header_len(pskb) - sizeof(struct udphdr) - payload_offset;
        memcpy(actual_data + strlen(actual_data), data_begin, data_length);

        //update IP len & udp len & checksum & frame len
        iplen = ntohs(iph->tot_len);
        iph->tot_len = htons(ntohs(iph->tot_len) + sizeof(struct int_metadata));
        iph->check = 0;
        iph->check = ip_fast_csum((char*)iph, iph->ihl);

        udplen = ntohs(udp->len);
        udp->len = htons(ntohs(udp->len) + sizeof(struct int_metadata));
        udp->check = 0;

        skblen = pskb->len;
        pskb->len += sizeof(struct int_metadata);
    }

    tailroom = pskb->end - pskb->tail;
    if (tailroom >= strlen(actual_data))
    {
        memcpy(data_begin, actual_data, strlen(actual_data));
    }
    else
    {
        int expand_room = strlen(actual_data) - tailroom;
        pskb_expand_head(skb, 0, expand_room, GFP_ATOMIC);
        memcpy(data_begin, actual_data, strlen(actual_data));
    }
