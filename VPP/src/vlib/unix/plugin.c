  	...
  	for (i = 0; i < vec_len (plugin_path); i++)
    {
      	...
      	while ((entry = readdir (dp)))
		{
	  		...
	  		if (p == 0)
	    	{
	      		...
	      		pi->file_info = statb;
	      		pi->handle = 0;
	      		hash_set_mem (pm->plugin_by_name_hash, plugin_name, pi - pm->plugin_info);
	    	}
			next:
	  		;
		}
...
vlib_plugin_early_init (vlib_main_t * vm)
{
	...
	pm->logger = vlib_log_register_class_rate_limit ("plugin", "load", 0x7FFFFFFF /* aka no rate limit */ );
