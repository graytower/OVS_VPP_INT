			   ...
			   vlib_frame_t * frame, af_packet_if_t * apif)
{
    ...
    {
		vec_validate (apm->rx_buffers[thread_index], VLIB_FRAME_SIZE + n_free_bufs - 1);
    	n_free_bufs += vlib_buffer_alloc (vm, &apm->rx_buffers[thread_index][n_free_bufs],
			   		   VLIB_FRAME_SIZE);
  	...
  	while ((tph->tp_status & TP_STATUS_USER) && (n_free_bufs > min_bufs))
    {
		vlib_buffer_t *b0 = 0, *first_b0 = 0;
      	...
      	while ((tph->tp_status & TP_STATUS_USER) && (n_free_bufs > min_bufs) && n_left_to_next)
		{
	    	...
	    	{
	      		/* grab free buffer */
	      		u32 last_empty_buffer = vec_len (apm->rx_buffers[thread_index]) - 1;
	      		prev_bi0 = bi0;
	      		...
	      		if (PREDICT_FALSE (tph->tp_status & TP_STATUS_VLAN_VALID))
				{
		  			if (PREDICT_TRUE (offset == 0))
		    		{
		      			...
		      			vlan->priority_cfi_and_id = clib_host_to_net_u16 (tph->tp_vlan_tci);
		      			vlan->type = eth->type;
	  		...
	  		else
	    	{
	      		...
	      		vnet_feature_start_device_input_x1 (apif->sw_if_index, &next0, first_b0);
	    	}
