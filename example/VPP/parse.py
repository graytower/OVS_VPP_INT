...
class parse():
    def filter(self, pkt_raw):  # filter int packet
        ...
        if ethernet == 0x0800:
            ...
            if ipv4_info == 222:
                pkt = struct.unpack("!12s%ds" % (pkt_len - 14 - 20 - 12), pkt[1])
                # udp_info = self.parse_udp(pkt[0])

                pkt = struct.unpack("!22s%ds" % (pkt_len - 14 - 20 - 12 - 22), pkt[1])
                int_md_num = 1 
                if int_md_num != 0:
                    data = self.int_pro(int_md_num, pkt[0])
                    return data
    ...
    def int_pro(self, n, pkt_raw):
        pkt_len = len(pkt_raw)
        int_list = []
        pkt = struct.unpack("!IIII6B", pkt_raw)
        print 'ingress: ', pkt[1]
        print 'egress:', pkt[3]
        return int_list
